...
class DummyFewShotDataset:
    ...
    Yields tuples (lr_frames, hd_frames, sparse_indices) with shapes:
      lr_frames:  [1, 3, T, H, W]
      hd_frames:  [1, 3, N, H, W]  (N in {0,1,2})
    ...
    def __iter__(self):
        for _ in range(self.steps):
            ...
            # LR video in [-1, 1]
            lr_frames = torch.rand(B, C, T, H, W, device=self.device) * 2 - 1

            if N == 0:
                # empty HD and indices
                hd_frames = torch.empty(B, C, 0, H, W, device=self.device)
                ...
                # take the LR frames at those indices as "HD" (可加点噪声模拟差异)
                # gather expects [B, C, T, H, W] -> [B, C, N, H, W]
                # 我们先把时间维提到 dim=2 再 gather
                # 方式一：逐帧拼接
                hd_list = []
                for t_idx in idx_list:
                    hd_list.append(lr_frames[:, :, t_idx:t_idx+1, :, :])  # [1,3,1,H,W]
                hd_frames = torch.cat(hd_list, dim=2)  # [1,3,N,H,W]
