...
class FewShotVideoSRTrainer:
    ...
    def compute_loss(self, lr_frames, hd_frames, indices_mask):
        ...
        # 先做 scheduler 规定的输入缩放
        latent_in = self.pipe.scheduler.scale_model_input(z_t, t)     # <── 关键

        ## Prepare UNet input: concat noisy target with LR conditioning
        latent_model_input = torch.cat([latent_in, cond_lr_latents], dim=2)  # [1, T, 2*C, H_latent, W_latent]
        ...
        # latents = self.pipe.scheduler.step(vel_pred, t, latents).prev_sample
        ...
        vel_gt = sqrt_ab * gt_noise.to(self.data_type) - sqrt_oma * full_cond_latents.to(self.data_type)
